...
description: Install, update, verify, or start DeepSeek Harness with the public Veang Workbench plugin from npm. Use when a user asks to install DSH, install Veang Workbench, set up the DSH web profile, replace an old private DSH restore workflow, or verify a public installation. Installs DSH 0.1.1-rc.2 and veang-workbench 0.2.3 without migrating sessions, settings, projects, credentials, a desktop shell, or patched dependency files.
---
...
- Stop if the installed DSH version cannot be verified as `0.1.1-rc.2`.
